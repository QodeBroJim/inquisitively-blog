...
try:
    from boto3.s3.connection import S3Connection, OrdinaryCallingFormat
    from boto3.exception import S3ResponseError
    from boto3.s3.key import Key
except ImportError:
    raise ImproperlyConfigured(
        "Could not load boto3's S3 bindings. Please install boto3."
    )
...
class S3Boto3Storage(Storage):
    ...
    def _open(self, name, mode='rb'):
        ...
        return S3Boto3StorageFile(name, mode, self)
...
class S3Boto3Storage_AllPublic(S3Boto3Storage):
    """
    Same as S3Boto3Storage, but defaults to uploading everything with a
    ...
    2) Since we don't have to hit S3 for auth keys, this backend is much
       faster than S3Boto3Storage, as it makes no attempt to validate whether
       keys exist.
    ...
    def __init__(self, acl='public-read', *args, **kwargs):
        super(S3Boto3Storage_AllPublic, self).__init__(acl=acl,
