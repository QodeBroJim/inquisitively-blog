...
def access_list_signup(request):
    form = AccessTutorialSignup(request.POST or None)
    if request.method == 'POST':
        if form.is_valid():
            email_signup_qs = AccessTutorialSignup.objects.filter(email=form.instance.email)
            if email_signup_qs.exists():
                messages.info(request, "You are already subscribed.")
            else:
                subscribe(form.instance.email)
